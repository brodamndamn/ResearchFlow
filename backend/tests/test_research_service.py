...
async def test_service_persists_plan_resume_report_sources_and_metrics(tmp_path: Path) -> None:
    ...
    assert completed.events[-1].phase == "completed"
    assert completed.events[-1].status == "completed"
...
async def test_quick_mode_rejects_more_than_two_review_queries(tmp_path: Path) -> None:
    from app.research_service import InvalidResearchState
    ...
    try:
        await service.update_plan(
            ...
            ResearchPlan(
                focus="关注成本边界",
                subqueries=["问题一", "问题二", "问题三"],
            ),
        )
    except InvalidResearchState as error:
        assert "快速模式最多允许 2 个子问题" in str(error)
    else:
        raise AssertionError("快速模式不应接受三个子问题")

    await queue.shutdown()
    await engine.dispose()


async def test_full_queue_restores_quota_and_removes_rejected_run(tmp_path: Path) -> None:
    engine = create_engine(tmp_path / "full.sqlite3")
    await initialize_database(engine)
    sessions = create_session_factory(engine)
    settings = Settings(
        _env_file=None,
    ...
    queue = BoundedTaskQueue(active_limit=1, waiting_limit=0)
    blocker = asyncio.Event()
    await queue.submit("blocker", blocker.wait)
    service = ResearchService(settings, sessions, FakeGraph(), queue)
